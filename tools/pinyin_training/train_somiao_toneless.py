...
def load_data(max_samples=200000):
    ...
    data = []
    # Dataset uses first row as column names - hanzi column and pinyin column
    col_names = dataset.column_names
    hanzi_col = col_names[0]  # First column is hanzi (e.g., '我们试试看！')
    pinyin_col = col_names[1]  # Second column is pinyin (e.g., 'wo3 men shi4 shi4 kan4 ！')
    print(f"Using columns: hanzi='{hanzi_col}', pinyin='{pinyin_col}'")

    for item in tqdm(dataset, desc="Processing"):
        # Get and normalize pinyin (remove tones)
        pinyin = normalize_pinyin(item[pinyin_col])
        hanzi = item[hanzi_col]
...
def main():
    ...
    # Detect device and set batch size accordingly
    if torch.cuda.is_available():
        device = torch.device('cuda')
        # For RTX 4090, use larger batch size for faster training
