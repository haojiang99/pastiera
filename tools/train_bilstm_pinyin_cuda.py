...
Optimized for NVIDIA RTX 4090 GPU with 24GB VRAM.

Features:
- Mixed precision training (BF16 default for Ada GPUs)
- Large batch sizes (512) optimized for 4090's 24GB VRAM
- Gradient accumulation for effective larger batches
- torch.compile() enabled by default for PyTorch 2.x
- Bucket batching by sequence length (reduces padding overhead)
...
Usage:
    python train_bilstm_pinyin_cuda.py --dataset ../pinyin_dataset.json

    # Resume from checkpoint:
    python train_bilstm_pinyin_cuda.py --resume checkpoints/checkpoint_epoch_10.pt

    # Disable compile for debugging:
    python train_bilstm_pinyin_cuda.py --no-compile
"""
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=128,
                       help='Batch size (128 for 4090 24GB)')
