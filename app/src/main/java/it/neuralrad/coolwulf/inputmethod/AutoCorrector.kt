...
object AutoCorrector {
        ...
        try {
            ...
            for (locale in standardLocales) {
                try {
                    ...
                    if (context != null) {
                        val customCorrections = it.neuralrad.coolwulf.SettingsManager.getCustomAutoCorrections(context, locale)
            ...
            if (context != null) {
                try {
                    val prefs = it.neuralrad.coolwulf.SettingsManager.getPreferences(context)
                    ...
                    for ((key, value) in allPrefs) {
                        if (key.startsWith("auto_correct_custom_") && value is String) {
                            ...
                            if (languageCode !in standardLocales) {
                                try {
                                    val customCorrections = it.neuralrad.coolwulf.SettingsManager.getCustomAutoCorrections(context, languageCode)
                                    if (customCorrections.isNotEmpty()) {
                                        val customJson = correctionsToJson(customCorrections)
    ...
    fun getCorrection(word: String, locale: String? = null, context: Context? = null): String? {
        ...
        if (context != null) {
            val enabledLanguages = it.neuralrad.coolwulf.SettingsManager.getAutoCorrectEnabledLanguages(context)
        ...
        if (targetLocale != "en") {
            ...
            if (context == null || it.neuralrad.coolwulf.SettingsManager.isAutoCorrectLanguageEnabled(context, "en")) {
                corrections["en"]?.let { enCorrections ->
                    enCorrections[wordLower]?.let { correction ->
    ...
    fun processText(
        ...
        val enabledLanguages = if (context != null) {
            it.neuralrad.coolwulf.SettingsManager.getAutoCorrectEnabledLanguages(context)
        } else {
            emptySet<String>()
        }
