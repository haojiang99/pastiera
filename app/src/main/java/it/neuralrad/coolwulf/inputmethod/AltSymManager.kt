...
class AltSymManager(
    ...
    fun reloadSymMappings() {
        if (context != null) {
            val customMappings = it.neuralrad.coolwulf.SettingsManager.getSymMappings(context)
    ...
    fun reloadSymMappings2() {
        if (context != null) {
            val customMappings = it.neuralrad.coolwulf.SettingsManager.getSymMappingsPage2(context)
            if (customMappings.isNotEmpty()) {
                symKeyMap2.clear()
